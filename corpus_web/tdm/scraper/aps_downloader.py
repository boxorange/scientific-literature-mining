...
class APSDownloader(BaseDownloader):
	...
	def __init__(self):
		self.destination = BaseDownloader.archive_path + "APS/"
	...
	def retrieve_all_open_access_articles(self):
		...
		for doi in duplicate_removed_uids:
			...
			if response.status_code == 200:
				...
				self.write_to_file(response, self.destination, filename, ".xml")
			else:
				self.display_error_msg(response)
				sys.exit()
	...
	def retrieve_articles(self, query):
		...
		for doi, link in doi_link.items():
			...
			if response.status_code == 200:
				self.write_to_file(response, self.destination, filename, ".zip")
			else:
				for l in link:
					url = l.get('URL')
					...
					if response.status_code == 200:
						...
						self.write_to_file(response, self.destination, filename, ext)
					else:
						self.display_error_msg(response)
						sys.exit()
