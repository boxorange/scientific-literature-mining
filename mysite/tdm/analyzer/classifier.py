...
def find_xas_result(elements, body_text):
	"""
	Test cases:
	...
	for elem in elements:	# figure_caption
		fig_id = elem['fig_id']
		sent = elem['sent']
			...
			for idx in edge_idx:
				...
				for idx, type_of_edge in cat_idx.items():
					...
					if region and element:
						...
						for r in region:
							for e in element:
								ret_val.append({'region': r, 'element': e, 'edge': type_of_edge})
			...
			for file in files:
				if file.endswith(".json"):
					...
					with open(os.path.join(root, file), "r+") as f:
						...
						results = find_xas_result(figure_caption, body_text)
